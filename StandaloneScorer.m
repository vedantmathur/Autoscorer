function file_handle = StandaloneScorer(cscfilename,timestamp,EEG_HP,EEG_LP)
    % EEG
    CSC_file = cscfilename;
    % Scored File <- No use!
    % TR_file = 'DATA/TR_1697_JH_1st24hrs';
    % Timestamps (Comes from Timestamp-Generator) 
    TS_file = timestamp;
    % Frequencies desired
    % EEG_Fc, EEG_HP_Fc
    EEG_Fc = EEG_LP;
    ...
    [EEG_samples, EEG_timestamps, reducedSamplingRate] = loadEEGData(CSC_file, timestampSections, EEG_Fc, EEG_HP_Fc);
    ...
    %dlmwrite('abc.txt',EEG_samples, 'newline', 'pc')
    ...
    % Get the index of the max frequency and taper the frequency axis to
    % account
    ...
    naming = date;
    resultsFilename = strcat(naming,'PSD.xlsx');
    ...
    clear columnHeaders
    xlswrite(resultsFilename,faxis, sheetName, 'C1');
    %Write results to the 'PowerSpectra' sheet:
    xlswrite(resultsFilename,timeline', sheetName, 'A3');
    xlswrite(resultsFilename,scoredEpochPowerSpectrum, sheetName, 'B3');
